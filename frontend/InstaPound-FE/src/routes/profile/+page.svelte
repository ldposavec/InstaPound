			...
			<Card variant="elevated" padding="lg" class="mb-8">
				...
				{#if authStore.user.pendingPackageType}
					<Alert variant="info" class="mb-6">
						<strong>Package change pending:</strong> Your plan will change to {authStore.user.pendingPackageType} on {formatDate(authStore.user.packageChangeEffectiveDate)}.
					</Alert>
				{/if}
					...
					<div class="rounded-xl bg-slate-50 p-4 dark:bg-slate-800">
						<div class="mb-3 flex items-center justify-between">
							...
							<span class="text-sm font-medium text-slate-900 dark:text-white">
								{packageStore.currentPackage.currentUsage?.todayUploadCount || 0} / {packageStore.currentPackage.limits.dailyUploadLimit === -1 ? '∞' : packageStore.currentPackage.limits.dailyUploadLimit}
							</span>
						</div>
						<div class="h-2 overflow-hidden rounded-full bg-slate-200 dark:bg-slate-700">
							<div
								...
								style="width: {packageStore.currentPackage.limits.dailyUploadLimit === -1 ? 0 : getUsagePercentage(packageStore.currentPackage.currentUsage?.todayUploadCount || 0, packageStore.currentPackage.limits.dailyUploadLimit)}%"
							></div>
						</div>
					</div>
