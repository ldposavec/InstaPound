import type { Photo, Page, PhotoSearchRequest, PhotoEditRequest, ImageFormat, ImageFilter, StorageType } from '$lib/types';
...
function createPhotosStore() {
	...
	async function getById(id: number) {
		...
		try {
			...
			error = e instanceof ApiError ? e.message : 'Failed to load photo';
			currentPhoto = null;
		} finally {
			loading = false;
		}
	...
	async function edit(id: number, data: PhotoEditRequest): Promise<Photo> {
		loading = true;
		error = null;
		try {
			...
			error = e instanceof ApiError ? e.message : 'Edit failed';
			throw e;
		} finally {
			loading = false;
		}
