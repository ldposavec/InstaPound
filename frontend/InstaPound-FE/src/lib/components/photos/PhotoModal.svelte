<script lang="ts">
	...
	const canEdit = $derived(
		authStore.isAuthenticated && (authStore.user?.id === photo.authorId || authStore.isAdmin)
	);
</script>
			...
			<div class="mb-6 rounded-xl bg-slate-50 p-4 dark:bg-slate-800">
				...
				<div class="space-y-2 text-sm">
					...
					<div class="flex justify-between">
						...
						<span class="max-w-[150px] truncate font-medium text-slate-900 dark:text-white">{photo.originalFileName}</span>
					</div>
				</div>
			</div>
