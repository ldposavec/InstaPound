...
@Service
@Slf4j
public class ImageProcessorServiceImpl {
    ...
    private BufferedImage applyInvert(BufferedImage image) {
        ...
        for (int x = 0; x < image.getWidth(); x++) {
            for (int y = 0; y < image.getHeight(); y++) {
                ...
                result.setRGB(x, y, new Color(255 - color.getRed(), 255 - color.getGreen(), 255 - color.getBlue())
                        .getRGB());
            }
        }
        return result;
    ...
    private BufferedImage applyResize(BufferedImage image, ImageProcessingOptions options) {
        if (options.getWidth() != null && options.getHeight() != null) {
            return Scalr.resize(image, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.FIT_EXACT,
                    options.getWidth(), options.getHeight(), Scalr.OP_ANTIALIAS);
        } else if (options.getWidth() != null) {
            return Scalr.resize(image, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.FIT_TO_WIDTH,
                    options.getWidth(), Scalr.OP_ANTIALIAS);
        } else if (options.getHeight() != null) {
            return Scalr.resize(image, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.FIT_TO_HEIGHT,
    ...
    public byte[] createThumbnail(InputStream inputStream, int maxWidth, int maxHeight) throws IOException {
        ...
        BufferedImage thumbnail = Scalr.resize(image, Scalr.Method.ULTRA_QUALITY, Scalr.Mode.AUTOMATIC,
